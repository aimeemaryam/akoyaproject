...
const Page5 = () => {
  ...
  return (
    <section 
      dir={isRtl ? 'rtl' : 'ltr'}
      className="w-full bg-[#FAF9F7] py-20 px-9 md:px-24 text-gray-900"
    >
      <div className="max-w-6xl mx-auto flex flex-col items-center">
        
        {/* Header Section */}
        <div className="text-center mb-14">
          <h2 className="text-2xl md:text-4xl font-light tracking-wide mb-4 text-[#1a1a1a]">
            {content.heading}
          </h2>
        ...
        {/* 3 Cards Grid */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 w-full mb-16 items-stretch">
          {content.cards.map((card) => (
            <div
              key={card.id}
              className="bg-white rounded-3xl overflow-hidden shadow-md hover:shadow-2xl hover:scale-105 transition-all duration-400 flex flex-col justify-between border border-gray-100/80 cursor-pointer"
            >
              {/* Image Container */}
              <div className="relative w-full h-[360px] bg-white flex items-center justify-center overflow-hidden">
                <img
                  src={card.image}
                  alt={card.title}
                ...
                {/* Floating Badge */}
                <div className="absolute bottom-4 ltr:left-4 rtl:right-4 bg-[#E5C158] text-black font-semibold text-xs px-4 py-1.5 rounded-full shadow-md z-10">
                  {card.badge}
                </div>
              </div>
              ...
              <div className="p-7 flex flex-col flex-grow justify-between">
                <div>
                  ...
                  <p className="text-gray-800 text-sm mb-6 font-normal">
                    {card.description}
                  </p>
                  ...
                  <ul className="space-y-3">
                    {card.features.map((feature, idx) => (
                      <li key={idx} className="flex items-center text-sm text-gray-800">
                        <svg
                          className="w-4 h-4 text-[#D4AF37] ltr:mr-2 rtl:ml-2 flex-shrink-0"
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
        ...
        <button
          ...
          className="bg-[#1C1C1E] hover:bg-black text-white font-semibold text-sm px-8 py-4 rounded-full shadow-md hover:shadow-xl hover:scale-105 transition-all duration-300 flex items-center gap-3 cursor-pointer"
        >
