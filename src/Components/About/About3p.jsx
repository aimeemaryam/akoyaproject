...
const About3p = () => {
  ...
  return (
    <section className="w-full bg-[#F8F5F2] py-18 px-8 md:px-24">
      {/* Section Title */}
      <div className="max-w-4xl mx-auto mb-12 text-center">
        <h2 className="text-3xl md:text-4xl font-light text-black tracking-wide">
          {lang === 'ar' ? 'رحلة خدماتنا' : 'Our Service Journey'}
        </h2>
      </div>

      {/* Grid Container with Left Alignment */}
      <div className="max-w-8xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {journeySteps.map((item) => (
          <div
            key={item.id}
            className={`bg-white rounded-2xl px-5 flex flex-col items-start shadow-sm border border-gray-100 min-h-[180px] justify-start ${
              lang === 'ar' ? 'text-right items-end' : 'text-left items-start'
            }`}
          >
            {/* Left/Right Aligned Icon Container */}
            <div className="mb-4 mt-6">{item.icon}</div>

            {/* Step Title */}
            <h3 className="text-lg px-4  font-medium text-gray-900 mb-2">
              {item.step} {lang === 'ar' ? item.titleAr : item.titleEn}
            </h3>

            {/* Step Description */}
            <p className="text-gray-800 px-4 text-[14px]">
